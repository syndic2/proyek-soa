...
const endpoint= '/api/recipes/searchByIngredients';
const method= 'GET';
...
it('Not passed (without parameter query)', (done) => {
    chai.request(host)
        ...
        .end((err, res) => {
            ...
            res.body.should.have.property('message').eql('Parameter key dan ingredient tidak boleh kosong.');
        done();
        });
}).timeout(10000);
...
it('Not passed (without valid key)', (done) => {
    chai.request(host)
        .get(`${endpoint}?key=test&ingredient=pork,noodle&limit=3`)
        .set('x-access-token', token)
        .end((err, res) => {
            res.should.have.status(401);
            res.body.should.be.a('object');
